class Game {
  ...
  createBoard() {
    ...
    // board['masterScores'] = [...Array(playerCount).fill(0)];
  ...
  createOrdering(flag = 0) {
    // return array with order of player by index 
    let order = [...Array(this.playerCount).keys()];
    // use playedHands to rotate array that many times
    let count = 0;
  ...
  enterBets(bets) {
   ...
   //  let index = this.playerStartFirst;
  	// for (let count = 0; count < this.playerCount; count++) {
  	//   roundBets[index] = bets[count];
  ...
  enterGets(gets) {
    ...
    console.log('enterGets');
    console.log(roundGets);
  }  
}
